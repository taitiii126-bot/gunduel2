...
ティアはCPU戦（ブラウザの中）の結果なので、完全には検証できません。そのため次の制限を付けています。
...
- CPU戦の成績とティアは、ログイン中ならプロフィールと一緒にサーバーにも保存されます（別の端末でも同じになる）。ただし自己申告なので公式記録（オンライン戦績）とは分けて扱います
...
- **称号（35種類）**：一覧は `sim.js` の `TITLES`、名前と取り方は `lang.js` の `title.〇〇` / `tcond.〇〇`。レアの称号はカードで青いネオンに光ります。オンライン戦績・フレンド数・開拓者（最初の100人）の称号はサーバーが確かめるので、条件を満たしていないと相手には初期の称号が見えます。それ以外（CPU戦など）は自己申告です
